...
import com.odenzo.xrpl.signing.crypto.seeds.RFC1751Keys.{ binaryToWord, wordToken2ByteVector }
import com.odenzo.xrpl.signing.models.XrpSeed
...
object RFC1751Keys {
  ...
  /**
    * Bytes to English String -- exactly how many bytes this should be?
    * Typically 64 bits done twice, so in our case strData should be 8 bytes
    * always
    */
  def binaryToWord(strData: ByteVector): String = {
    val caBuffer: ByteVector   = strData.take(8)
    val fullBuffer: ByteVector = caBuffer ++ hex"0000"
  ...
  /**
    * Convert RFC1751's six words to Binary data in tortured Scala mutable code.
    *
    * @return
    *   11 byte ByteVector
    */
  private[signing] def wordToken2ByteVector(vsHuman: List[String]): ByteVector = {
    assert(vsHuman.length === 6)

    // These bytes will be mutated in the array itself
    val ab: ArrayBuffer[Byte] = ArrayBuffer.fill[Byte](11)(0.toByte)

    /** Re-write with ByteVector once this passes unit tests. */
    val binary: List[Byte] = vsHuman
      .zipWithIndex
      .map {
        case (strWord, index) =>
      ...
      }.flatMap((_: ArrayBuffer[Byte]) => ab)

    val resultBinary = ByteVector(binary)
    // Rats, I have seemed to munge this.

    val parity = computeParity(resultBinary)
    if ((parity & 3) =!= extract(resultBinary, 64, 2).toInt) then
      throw IllegalArgumentException(s"Parity Check Failed on RFC1751 Keys")
    else resultBinary.take(8) // Take 8 bytes

  }

  /**
    * 16 bytes to twelve words. Note this reverses the order of bytes per
    * Ripple. Some examples on net don't seem to do this.
    */
  def bytesToEnglish(seed: ByteVector): Any = {

    val bytes: ByteVector = seed.reverse
    val upper: String     = binaryToWord(bytes.take(8))
    val lower: String     = binaryToWord(bytes.drop(8).take(8))
    upper + " " + lower

  }

  /**
    * Ripple case, 12 RFX words to 128 bits/16 bytes ByteVector (seed, can turn
    * to seed_hex)
    */
  def twelveWordsAsBytes(strHuman: String): XrpSeed = {
    val words: List[String] = strHuman.trim().split(" ").toList
    if (words.length =!= 12) throw IllegalArgumentException(s"RFC1751 not 12 works: ${words.length}")
    else {
      val strFirst: ByteVector  = wordToken2ByteVector(words.take(6))
      val strSecond: ByteVector = wordToken2ByteVector(words.drop(6))
      val strKey: ByteVector    = strFirst ++ strSecond
      XrpSeed.fromBytesUnsafe(strKey.reverse)
    }
  }

  /**
    * This does takes a subsection of the bits/bytes and returns as a Long
    * value. Sometimes we want to extract 2 bytes
    *
    * * @param s List if Bytes
    * @param start
    *   Start index (in bits not bytes !?)
    * @param length
    *   Length in bytes (Bits?)
    */
  protected def extract(s: ByteVector, start: Int, length: Int): Long = {

    // &0xFF is just masking and don't understand why needed.
    // Because we are putting a byte into an Int
    val startBitOrByte: Int = start / 8
    // We are getting bytes and essentially making an unsigned int.
    // Seems clearer with cl cc cr being BitVectors 3 bytes long
    val cl: Int             = s(startBitOrByte) & 0xff // get components
    val cc: Int             = s(startBitOrByte + 1) & 0xff
    val cr: Int             = s(startBitOrByte + 2) & 0xff

    // Then treat as a number
  ...
  /**
    * @param bytes
    *   Not sure the prerequisites on length but its 64 bytes and we do two
    *   bytes at a time.
    *
    * @return
    */
  protected def computeParity(bytes: ByteVector): Int = {
  ...
  /**
    ...
    * @param x
    * @param start
    *   Start and length to not overlap it seems
    * @param length
    *
    * @return
    *   Mutated s, incoming bytes are also mutated
    */
  ...
  /**
    * @param wordlist
    *   This is always WORDLIST above for non-testing
    * @param word
    *   The word to find.
    * @param startIndex
    *   This is always zero in our scenario
    *
    * @return
    *   The index of given word in the wordlist, 0 based. -1 if not found
    */
  protected def indexOf(wordlist: Seq[String], word: String, startIndex: Int = 0): Int = {
    wordlist.indexOf(word, startIndex)
  }

  /** Just cleans up a word before processing substituting confusing things. */
  private[signing] def normalizeWord(strWord: String): String = {
    strWord.toUpperCase.map {
      case '1'   => 'L'
      case '0'   => 'O'
      case '5'   => 'S'
      case other => other
